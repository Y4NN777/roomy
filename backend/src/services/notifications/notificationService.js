...
class NotificationService {
  // Initialize the service, check email config, set up event listeners.
  constructor() {
  ...
  // Register event listeners for tasks, expenses, groups, and AI events.
  setupEventListeners() {
    ...
    eventBus.on(EventTypes.GRrequireOUP_EMAIL_INVITATION_SENT, (data) => this.handleGroupEmailInvitationSent(data));
  ...
  async sendEmail(to, subject, html, text) {
    ...
    if (!transporter) {
      logger.error('Email transporter is null');
      return { 
        success: false, 
  ...
  async sendGroupInvitation(data) {
    ...
    try {
      ...
      if (result.success) {
        ...
        logger.warn(`Group invitation failed to ${recipientEmail}:`, result.reason || result.error);
      }
  ...
  // Notify a user by email about a role change in a group.
  ...
  // Send an in-app notification (WebSocket integration placeholder).
...
// Core notification creation and delivery
  /**
   * @method createAndDeliverNotification
   * @description Creates a notification in the database and delivers it in real-time via WebSocket.
   ...
   * @returns {Promise<object>} - A promise that resolves with the list of notifications and pagination details.
   */
  ...
  // Mark all notifications as read for a user (optionally by group)
   ...
   * @description Gets the count of unread notifications for a user, optionally filtered by group.
   * @param {string} userId - The ID of the user.
   * @param {string|null} [groupId=null] - The optional group ID to filter by.
  ...
  /**
   * @method cleanup
   * @description Removes all event listeners to allow for graceful shutdown.
   */
  // Remove all event listeners for graceful shutdown.
  cleanup() {
